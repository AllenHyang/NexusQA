...
(async () => {
  ...
  for(let i=0; i<10; i++) {
      try {
        ...
        console.log(`\x1b[33m[Watcher] Waiting for server... (${i+1}/10)\x1b[0m`);
        await new Promise(r => setTimeout(r, 2000));
      }
  }
  
  if (!connected) {
       console.error(`\x1b[31m[Watcher] Failed to connect to ${TARGET_URL} after retries.\x1b[0m`);
       // Keep browser open so user can see what happened or try manual refresh
  }
})();
