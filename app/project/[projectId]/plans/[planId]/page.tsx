...
import { ArrowLeft, CheckCircle2, XCircle, AlertCircle, Calendar, Search } from "lucide-react";
...
export default function PlanDetailPage() {
  ...
  const { currentPlan, fetchPlan, updateRunStatus } = useAppStore();
                      ...
                      {filteredRuns.map(run => (
                          <tr key={run.id} className="hover:bg-zinc-50/50 transition-colors">
                              <td className="px-6 py-4">
                                  <div className="font-bold text-zinc-700">{run.testCase?.title || "Unknown Case"}</div>
                                  {run.notes && <div className="text-xs text-zinc-400 mt-1 italic">{run.notes}</div>}
                              </td>
                              <td className="px-6 py-4">
                                  <PriorityBadge priority={run.testCase?.priority || "MEDIUM"} />
                              </td>
                              <td className="px-6 py-4">
