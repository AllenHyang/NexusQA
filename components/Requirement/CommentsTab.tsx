...
export function CommentsTab({ requirementId, currentUser, topic, compact = false }: CommentsTabProps) {
  ...
  // 回复评论 - 自动 @ 该用户
  const handleReply = (comment: RequirementComment) => {
          ...
          comments.map((comment) => (
            <div key={comment.id} className="p-3 bg-zinc-50 rounded-xl border border-zinc-100">
              ...
              {editingId === comment.id ? (
                ...
                <div>
                  <div className="text-sm text-zinc-700 whitespace-pre-wrap">{renderWithMentions(comment.content)}</div>
                  <button
                    onClick={() => handleReply(comment)}
                    className="mt-2 flex items-center gap-1 text-xs text-zinc-400 hover:text-blue-600 transition-colors"
                  >
